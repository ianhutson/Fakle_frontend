...
class RollableDice extends Component{
  constructor(props) {
    ...
    for (let i = 1; i <= this.props.store.rollable_dice; i++) {    
      ...
      dice.push(
        <div>
        <img className={include_check ? "dice_background_active" : "dice_background"} onClick={this.handleOnClick.bind(this, i)} src={process.env.PUBLIC_URL + dicearr[this.props.store.rolled_dice[i-1]-1]} />
        <br></br><br></br>
        </div>  
      )
    }
